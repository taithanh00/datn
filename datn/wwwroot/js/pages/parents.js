...
function setupEventListeners() {
    // Create Button
    document.getElementById('btnCreateParent').addEventListener('click', openCreatePanel);
    ...
    document.getElementById('linkModalOverlay').addEventListener('click', closeLinkStudentModal);
}
        ...
        row.innerHTML = `
            ...
            <td>
                ${p.children && p.children.length > 0
                    ? p.children.map(c => `<span class="child-badge" title="${c.relationship}">${c.fullName}</span>`).join('')
            ...
            <td class="text-end">
                <div class="table-actions">
                    <button class="btn-icon" onclick="openEditPanel(${p.id})" title="Chỉnh sửa">
                        <i class="fa-solid fa-pen-to-square"></i>
                    </button>
                    <button class="btn-icon btn-delete" onclick="deleteParent(${p.id}, '${p.fullName}')" title="Xóa">
...
function openCreatePanel() {
    ...
    document.getElementById('passwordNote').textContent = 'Mặc định: 123456';
...
async function openEditPanel(id) {
    ...
    try {
        ...
        document.getElementById('avatarPreview').src = p.avatarPath || '/images/lion_orange.png';
        document.getElementById('password').value = '******';
        document.getElementById('passwordNote').textContent = 'Để trống nếu không thay đổi mật khẩu';

        document.getElementById('panelTitle').textContent = `Sửa thông tin - ${p.firstName} ${p.lastName}`;
...
async function deleteCurrentParent() {
    ...
    const name = document.getElementById('panelTitle').textContent.replace('Sửa thông tin - ', '');
    await deleteParent(currentParentId, name);
}

async function deleteParent(id, name) {
    if (!confirm(`Bạn có chắc chắn muốn xóa phụ huynh "${name}"? Thao tác này cũng sẽ xóa tài khoản đăng nhập của họ.`)) {
        return;
    }

    try {
        const response = await fetch(`/Manager/Api/Parent/${id}`, { method: 'DELETE' });
        const result = await response.json();
...
function renderLinkedStudents(children) {
    ...
    if (!children || children.length === 0) {
        container.innerHTML = '<p class="text-muted small text-center" style="padding: 12px 0;">Phụ huynh này chưa được liên kết với học sinh nào.</p>';
        return;
    }
...
async function searchStudentsForLink(q) {
    try {
        const response = await fetch(`/Manager/Api/Students/Search?q=${encodeURIComponent(q)}`);
        const result = await response.json();
        if (result.success) {
            const list = document.getElementById('studentSearchResults');
            if (result.data.length === 0) {
                list.innerHTML = '<div style="padding: 12px; text-align: center; color: var(--text-muted); font-size: 0.85rem;">Không tìm thấy học sinh nào</div>';
                return;
            }
            list.innerHTML = result.data.map(s => `
                <a href="javascript:void(0)" class="list-group-item list-group-item-action" onclick="selectStudentForLink(${s.id}, '${s.fullName}', '${s.code}')"
...
async function confirmLinkStudent() {
    ...
    try {
        const response = await fetch('/Manager/Api/Parent/LinkStudent', {
            ...
            body: `parentId=${currentParentId}&studentId=${selectedStudentForLink}&relationship=${encodeURIComponent(relationship)}`
        });
        const result = await response.json();
        if (result.success) {
            closeLinkStudentModal();
            openEditPanel(currentParentId); // Refresh linked list
        ...
        showFormAlert('Lỗi khi liên kết', 'error');
    }
}
...
async function unlinkStudent(studentId) {
    ...
    try {
        const response = await fetch(`/Manager/Api/Parent/UnlinkStudent?parentId=${currentParentId}&studentId=${studentId}`, { method: 'DELETE' });
        const result = await response.json();
        if (result.success) {
